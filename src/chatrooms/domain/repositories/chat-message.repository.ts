...
export interface ChatMessageRepository {
  ...
  create(chatroomId: string, data: Omit<ChatMessage, 'id' | 'createdAt' | 'updatedAt'>): Promise<ChatMessage>;
  update(chatroomId: string, id: string, data: Partial<Omit<ChatMessage, 'id' | 'createdAt'>>): Promise<ChatMessage | null>;
  delete(chatroomId: string, id: string): Promise<void>;
  findByChatroom(chatroomId: string): Promise<ChatMessage[]>;
  addReaction(chatroomId: string, id: string, userId: string, reaction: UpdateMessageReactionDto): Promise<ChatMessage | null>;
