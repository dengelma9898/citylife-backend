...
export interface NewsFileValidationOptions {
  optional?: boolean;
}

@Injectable()
export class NewsFileValidationPipe implements PipeTransform {
  private readonly logger = new Logger(NewsFileValidationPipe.name);
  private readonly MAX_FILE_SIZE = 5 * 1024 * 1024; // 5MB for news images
  private readonly ALLOWED_MIME_TYPES = ['image/jpeg', 'image/png', 'image/jpg'];
  private readonly options: NewsFileValidationOptions;

  constructor(options: NewsFileValidationOptions = {}) {
    this.options = options;
  }

  transform(file: Express.Multer.File, metadata: ArgumentMetadata) {
    this.logger.debug(`Validating news file: ${file?.originalname}`);
    ...
    this.logger.debug('News file validation successful');
    return file;
  }
}
